	...
	verif();
		if(cas()!='ERREUR'){
			if (text_idfacture_client.text==''){
				var validators:Array = Validator.validateAll([valid_date_facture, valid_delai_paiement, valid_operation, valid_type, valid_libelle]);
...
private function appliquer():void {
	if (text_idfacture_client.text==''){
	var validators:Array = Validator.validateAll([valid_date_facture,  valid_delai_paiement, valid_operation, valid_type]);
	if(!validators.length) {
		createItem();
